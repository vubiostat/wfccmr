wfccmgui <- function(availableCriteria=NULL) {
    if (is.null(availableCriteria))
        availableCriteria <- c("asam", "asam.rank", "cvalue", "dvalue", "huwright", "huwright.rank", "info", "prob.c", "prob.c.fdr", "prob.c.rank", "prob.d", "prob.d.fdr", "prob.d.rank", "prob.t", "prob.t.fdr", "prob.t.rank", "sam", "tvalue", "wga", "wga.rank")
    else if (!is.character(availableCriteria))
        availableCriteria <- names(availableCriteria)

    addcriteria <- function(crit, oper, val) {
        if (tclvalue(oper) %in% c("DESC", "ASC")) {
    ...
    removecriteria <- function() { tkdelete(criterialb, tkcurselection(criterialb)) }
    ...
    fillsignedlb <- function() {
        for (option in criteriacboptions)
            tkinsert(signedlb, "end", option)
    }

    rmsignedlb <- function(...) {
        for (j in 1:length(criteriacboptions))
            tkdelete(signedlb, 0)

        for (option in criteriacboptions) {
            if (tclvalue(sign) != option) {
                tkinsert(signedlb, "end", option)
            }
        }
    }
    ...
    setOneTwoPreFilter <- function(num) {
        filter <- paste("(PctGrp1 >= ", num, ") | (PctGrp2 >= ", num, ")", sep="")
        PreFilter <<- tclVar(filter)
        tkconfigure(entry.PreFilter, textvariable=PreFilter)
        tkset(alldataspin, 0)
    }
    ...
    buildCriteria <- function() {
        ...
        repeat {
            if (tclvalue(tkget(criterialb, 0)) == "") {
                ...
                if (nameString == "fdrpass" && !fdrFlag) {
                    fdrPass <<- newCriteria@values
                    fdrFlag <- TRUE
                } else if (nameString == "numpass" && !numFlag) {
    ...
    buildManager <- function() {
        ...
        } else if (tclvalue(managertype) == "Cutoff" ) {
    ...
    stdfuncbut <- tk2button(tt, text="Generate STD Function", width=20, command=function() { getSTDFunc(criteriacboptions) })
